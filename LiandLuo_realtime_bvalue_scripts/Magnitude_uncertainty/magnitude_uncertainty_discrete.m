% The script describes how nonuniform magnitude errors bias the b-value estimation for the three methods (discrete magnitudes)
clc,clear
...
parfor i=1:Sim
    ...
    Magn=Magn((Magn-Mc)>=-1e-6);
    %MLE   
    b_mle(i)=1/(log(10)*(mean(Magn)-Mc+delta));

    %b-positive
    Dm0=diff(Magn);
    jkf3=find((Dm0-Mc_prime)>=-1e-6);
    Dm=Dm0(jkf3);
    b_pos(i)=1/delta/log(10)*acoth((mean(Dm)-Mc_prime+delta )/delta); 
