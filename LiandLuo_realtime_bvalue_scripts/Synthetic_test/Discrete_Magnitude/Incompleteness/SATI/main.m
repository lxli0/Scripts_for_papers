...
for i=1:Sim
    ...
    for j=1:length(tdelete)
        ...
        number_kms(i,j)=length(Magn((Magn-Mc_kms(i,j))>=-1e-6));
...
subplot(3,1,1)
errorbar(tdelete_plot-0.05,meanb_mle,stdb_mle,'o','color',color(1,:),'linewidth',2)
hold on;
errorbar(tdelete_plot,meanb_pos,stdb_pos,'o','color',color(2,:),'linewidth',2)
hold on;
errorbar(tdelete_plot+0.05,meanb_kms,stdb_kms,'o','color',color(3,:),'linewidth',2)
...
subplot(3,1,2)
yyaxis left
errorbar(tdelete_plot-0.05,meanMc_mle,stdMc_mle,'o','color',color(1,:),'linewidth',2)
hold on;
yyaxis right
...
ylim([0 0.6])
yyaxis left
errorbar(tdelete_plot+0.05,meanMc_kms,stdMc_kms,'o','color',color(3,:),'linewidth',2)
set(gca,'fontsize',16)
xlim([0.5,4.5])
set(gca,'xticklabel',[]);
ylabel('Mc')
...
subplot(3,1,3)
errorbar(tdelete_plot-0.05,meannum_mle,stdnum_mle,'o','color',color(1,:),'linewidth',2)
hold on;
errorbar(tdelete_plot,meannum_pos,stdnum_pos,'o','color',color(2,:),'linewidth',2)
hold on;
errorbar(tdelete_plot+0.05,meannum_kms,stdnum_kms,'o','color',color(3,:),'linewidth',2)
set(gca,'fontsize',16)
xlim([0.5,4.5])
ylim([0 1400])
