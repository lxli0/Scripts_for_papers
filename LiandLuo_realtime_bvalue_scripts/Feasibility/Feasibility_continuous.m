%% The script describes how nonuniform magnitude errors bias the b-value estimation for the three method (continuous magnitude)
clc,clear
Sim=1e3;%number of synthetic catalogs
for i=1:Sim
    str=['./gen_catalog_continuous/',num2str(i),'.txt'];
    D=load(str);
    T=D(:,1)';
    M=D(:,2)';
    jkf=find(T>=1);
    ...
    [Mc_mle(i),b_mle(i),number_mle(i)]=MBS_MLE_continuous(Magn);
    [Mc_pos(i),b_pos(i),number_pos(i)]=MBS_pos_continuous(Magn);  
    Mc_kms(i)=Mc_mle(i);
    ...
    p_mle(i)=exp(-abs(aic_mle(i))/2-2);
    p_pos(i)=exp(-abs(aic_pos(i))/2-2);
    p_kms(i)=exp(-abs(aic_kms(i))/2-2);
    Sim-i
end
