...
P_large=1-(1-10.^(-b*m_range))/(1-10^(-b*m_trunc));
P_large(P_large<0)=0;
...
figure('units','normalized','position',[0.1,0.1,0.25,0.25])
 ...
 for i=1:L
     Dm=-1/b.*log10(1-rand(1,N)*(1-10^(-b*m_trunc)))-0.05; % generate random magnitude
...
hold on;
xticks([0:1:6]);
title('Truncated GR, {\itb} = 1, {\itm}_{truncated} = 3');
