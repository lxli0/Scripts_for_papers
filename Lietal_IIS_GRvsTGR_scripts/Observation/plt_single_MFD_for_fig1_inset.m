...
% mc)
function []=plt_single_MFD_for_fig1_inset(M,Dm,mc,b_GR,b_TGR,Mcorner_TGR_minus_mc,P_LR,alpha_GR,alpha_TGR)
    ...
    plot_cn0=semilogy(m,cn0,'s', 'color', 0.4*[1 1 1], ...
                'markersize', 4.5, 'markerfacecolor', color1(2,:), ...
    ...
    if isnan(mc)==0
       ...
       % fit_GR=semilogy(m+mc,length(Dm)*F_GR,'color',color2(3,:),'linewidth',3);
        hold on;
        fit_TGR=semilogy(m+mc,length(Dm)*F_TGR,'--','color',[0.85, 0.1, 0.1],'linewidth',1.5);
        hold on;
    ...
    textStr = sprintf('{\\itp}_{LRT} = %.2f', P_LR);
...
function [pl] = GR_confidential_interval(b, N, mc_set)
    ...
    y_fill = [n0, fliplr(n0)];
    fill(x_fill, y_fill,[0.2, 0.4, 0.9], ...
         'EdgeColor', 'none', 'FaceAlpha', 0.3);
    hold on;

    % Optionally plot the median line
    pl = plot(meadian + mc_set, n0, '-', 'Color',[0.2, 0.4, 0.9], 'LineWidth', 1.5);
    hold on;
end
