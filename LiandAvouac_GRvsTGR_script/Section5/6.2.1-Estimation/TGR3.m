...
for k=1:size1
    for j=1:size2
        ...
        mcorner_TGR(1:L/2)=log10(L)/b-1-lambda(k)/2;
        mcorner_TGR(L/2+1:L)=log10(L)/b-1+lambda(k)/2;
...
for k=1:size1
    for j=1:size2
        ...
        scatter(1/2,mean(mcorner_TGR),30,gradientColor,'filled');hold on;
        forleg((k-1)*size2+j) = arrayfun(@(x,y) ['$n=' num2str(x) ', \lambda=' num2str(y) '$'], L,lambda(k), 'UniformOutput', false);
    end
end
...
syms n b lambda i
equation = sprintf('$m_{corner}(i) = \\log_{10}(\\frac{n}{b}) - 1\\pm \\frac{\\lambda}{2}$');
...
syms n b lambda i
equation = sprintf('$m_{corner} =\\log_{10}(\\frac{n}{b}) - 1$');
