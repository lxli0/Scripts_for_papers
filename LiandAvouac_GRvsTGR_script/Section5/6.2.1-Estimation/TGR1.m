...
for j=1:size2
    ...
    color_plt=[color_plt;mainColors;ones(1,3)];
end
values_mco=values_mco(:,1:end-1);
values_b=values_b(:,1:end-1);
color_plt=color_plt(1:end-1,:);
...
legend(hh,{'$\beta=1$', '$\beta=0.5$'}, 'Interpreter', 'latex');
...
syms n b lambda i
equation = sprintf('$m_{corner} = \\frac{\\sum{\\log_{10}(\\frac{i}{b}) - \\beta}}{n}$');
