...
% Title
title('Coso', 'FontSize', 18, 'FontWeight', 'bold');

%}
%% Load data
D=load('Coso.txt'); % only Decatur: (2,3):(northing,easting) and Otaniemi: (2,3):(easting,northing); other lat,lon
mc_pre=1.5;
jkf=find(D(:,1)>=1981&D(:,1)<=2022);
D=D(jkf,:);
time_flag=1;
timeleg=['Year'];
...
% winthin Coso
jkf_A = find(D(:,2) > latMin & D(:,2) < latMax & D(:,3) > lonMin & D(:,3) < lonMax);
% Outside Coso
jkf_B = find(~(D(:,2) > latMin & D(:,2) < latMax & D(:,3) > lonMin & D(:,3) < lonMax));
...
%% operation
color_ope=[0 0 0.5;0.2 0.4 0.8;0.6 0.8 1];
Dope=load('Coso_operation.txt');
t_ope=Dope(:,1);
inj_op=Dope(:,2)/1e6;
out_ope=Dope(:,3)/1e6;
subplot(3,2,5)
yyaxis left
plot(t_ope,out_ope,'-','color',color_ope(1,:),'linewidth',2.5);
hold on;
plot(t_ope,inj_op,'-','color',color_ope(2,:),'linewidth',2.5);
hold on;
plot(t_ope,out_ope-inj_op,'-','color',color_ope(3,:),'linewidth',2.5);
hold on;
ylim([0 inf]);
xlabel('Year');
ylabel('Monthly Fluid Volume (m^3\times10^6)');
grid on;box on;grid minor;set(gca,'fontsize',16)
xlim([1981 2022])
%% EQ rate
color_mc=[ 0.8 0.2 0;1 0.6 0.2];
mc=1.5;
t4count=1981:0.1:2023;
...
yyaxis right
for j=1:2
    plot(t4count,EQ_count(:,j),'-','color',color_mc(j,:),'linewidth',2.5);hold on;
end
ylabel('Cumulative Earthquake Number')
legend('Injection','Production','Net','Total','m \geq 1.5','NumColumns',1,'location','Northwest');
ylim([0 1.4e4]);
