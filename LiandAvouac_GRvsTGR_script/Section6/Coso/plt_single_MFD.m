...
% mc)
function []=plt_single_MFD(M,Dm,mc,b_GR,b_TGR,Mcorner_TGR_minus_mc,P_LR)
    ...
    if isnan(mc)==0
        ...
        hold on;
        fit_TGR=semilogy(m+mc,length(Dm)*F_TGR,'color',color2(3,:),'linewidth',2.5);
        hold on;
       ...
       %         'Fit (T_G_R)','90%','Location','Northeast','NumColumns',1);
        ...
        textStr = sprintf('$m_c$: %.1f\\\\\n$b_{GR}$: %.2f\\\\\n$b_{TGR}$: %.2f\\\\\n$m_{corner}$: %.1f\\\\\n$p$: %.2f', mc, b_GR, b_TGR, Mcorner_TGR_minus_mc+mc,P_LR);
