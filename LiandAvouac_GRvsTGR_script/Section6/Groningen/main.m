...
close all
%% Load data
D=load('Groningen.txt'); % only Decatur: (2,3):(northing,easting) and Otaniemi: (2,3):(easting,northing); other lat,lon
jkf=find(D(:,1)>=1920);
D=D(jkf,:);
time_flag=1;
timeleg=['Year'];
...
Dope=load('Groningen_operation.txt');
t_ope=Dope(:,1);
out_ope=Dope(:,2)/1e8;
subplot(3,3,3)
yyaxis left
...
ylim([0 inf]);
xlabel('Year');
ylabel('Monthly Extraction (m^3\times10^8)');
grid on;box on;grid minor;set(gca,'fontsize',16)
%xlim([1981 2022])
%% EQ rate
color_mc=[ 0.8 0.2 0;1 0.6 0.2];
t4count=1991:0.1:2023.6;
...
legend('Extraction','Total','m \geq 1.3','NumColumns',1,'location','Northwest');
ylim([0 2e3]);
xlim([1960 2024]);
